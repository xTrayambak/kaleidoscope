...
when not noSimd:
  ...
  func findAvx2(haystack, needle: string): int {.inline.} =
    ...
    while i < haystack.len:
      if i + needle.len - 1 > haystack.len:
        break
      let
        blockFirst = mm256_loadu_si256(cast[ptr M256i](haystack[i].addr))
        blockLast =
          mm256_loadu_si256(cast[ptr M256i](haystack[i + needle.len - 1].addr))

        eqFirst = mm256_cmpeq_epi8(first, blockFirst)
        eqLast = mm256_cmpeq_epi8(last, blockLast)

      var mask: uint32 =
        cast[uint32](mm256_movemask_epi8(mm256_and_si256(eqFirst, eqLast)))
...
proc find*(haystack, needle: string): int {.inline.} =
  ## Find the position of the first occurence of `needle` in `haystack`.
  ##
...
proc contains*(haystack, needle: string): bool {.inline.} =
  ## This function returns `true` if `needle` is found in `haystack`.
